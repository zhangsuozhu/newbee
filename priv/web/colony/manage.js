...
export async function joinEnvironment() {
  let invitation;
  const validateInvite = (text) => {
    try {
      invitation = JSON.parse(atob(text.replace(/^newbee-colony:/, '')));
      if (!invitation || typeof invitation !== 'object' || Array.isArray(invitation) ||
          typeof invitation.code !== 'string' || !invitation.code.trim() ||
          typeof invitation.url !== 'string' || !['http:', 'https:'].includes(new URL(invitation.url).protocol)) {
        throw new Error('invalid');
      }
      return '';
    } catch (_) {
      invitation = null;
      return '环境邀请码无效，请粘贴「邀请另一台 AI 环境」生成的完整邀请码';
    }
  };
  const value = await form('加入蜂群', [{name:'invite',label:'环境邀请码',required:true,multiline:true,validate:validateInvite}, {name:'display',label:'这台环境的名称',required:true}, {name:'cwd',label:'允许工作的本地项目目录',required:true,help:'远端工作只交给此环境。连接断开后在安全边界暂停，不自动接管未知结果。'}], '加入');
  if (!value) return;
  try {
