...
function colonyGroup(item, ctx) {
  ...
  // 收起状态下也要能看到「有没有事等你」，不用先点开。
  const pending = stats.honey_pending_review || 0;
  ...
  header.innerHTML = `
    ...
    ${pending ? `<span class="session-group-need" title="${pending} 件成果等你验收">${pending} 待验收</span>` : ""}
    ${busy ? `<span class="session-group-busy" title="${busy} 只 Bee 正在干活">● ${busy} 在干活</span>` : ""}
...
function beeItem(m, colonyId, ctx) {
  ...
  const need = isMe ? 0 : urgency(m).need;
  item.innerHTML = `
    <span class="t"><span class="sess-dot ${dotClass(m)}"></span>${esc(m.display)}<span class="session-role">${esc(kindLabel(m.kind))}</span>${need ? `<span class="member-need" title="${need} 件事等你处理">${need}</span>` : ""}</span>
    <span class="meta" title="${esc(isMe ? "这是你自己" : metaLine(m))}">${esc(isMe ? "你自己" : metaLine(m))}</span>`;
...
// 谁需要人：等你答复的任务、待你验收的任务、被暂停的 AI。侧栏据此把该处理的人排到最前。
function urgency(m) {
  const open = ((state.data && state.data.tasks) || []).filter((t) => t.assigned_bee_id === m.id && !['done', 'failed', 'cancelled'].includes(t.status));
  const blocked = open.filter((t) => t.status === 'blocked').length;
  const review = open.filter((t) => t.status === 'pending_review').length;
  const paused = m.kind !== 'human' && m.control_state && m.control_state !== 'running' ? 1 : 0;
  const self = state.data && state.data.actor_bee_id === m.id ? 1 : 0;
  // 等你答复 → 待你验收 → 被暂停 → 在干活 → 空闲
  const rank = blocked ? 0 : review ? 1 : paused ? 2 : m.status === 'working' ? 3 : 4;
  return {rank, need: blocked + review, blocked, review, paused, self};
}

function orderMembers(members) {
  return [...members].sort((a, b) => {
    const x = urgency(a), y = urgency(b);
    if (x.self !== y.self) return x.self - y.self;   // 自己排最后
    if (x.rank !== y.rank) return x.rank - y.rank;   // 需要你 → 在干活 → 空闲
    if (x.need !== y.need) return y.need - x.need;   // 等你的事越多越靠前
    const human = (m) => (m.kind === 'human' ? 1 : 0);
    if (human(a) !== human(b)) return human(a) - human(b);
    return String(a.display || '').localeCompare(String(b.display || ''), 'zh');
  });
}
...
function metaLine(m) {
  ...
  else if (m.kind !== 'human' && m.control_state === 'pausing') bits.push('暂停中 · 等待确认');
  if (u.blocked) bits.push(`${u.blocked} 项等你答复`);
  else if (u.review) bits.push(`${u.review} 项待你验收`);
  const doing = currentTaskTitle(m.id);
  if (doing) bits.push(`在干：${doing}`);
  else if (m.status === 'working') bits.push('正在干活');
  else if (m.active_tasks) bits.push(`${m.active_tasks} 个任务在手`);
  else if (!u.blocked && !u.review) bits.push(statusLabel(m.status));
