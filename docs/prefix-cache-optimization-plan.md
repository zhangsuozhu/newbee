...
> 状态：**已实施**（2026-08，PR #6 envelope 命中链路修正 + 缓存命中诊断；
> 后续 PR #10/#18 加 cache-hit 日志统计）。
> 核心落位：`lib/newbee/request_envelope.ex`（可缓存前缀快照：消息+tools+route 逐字节，
> 摘要请求 = 快照严格前缀 + 尾部压缩指令才伪装命中）+ `lib/newbee/agent/loop.ex`（路由前记录）+ `lib/newbee/archive.ex`（摘要消费）。
