defmodule Newbee.Tools.Edit.V2 do
  @moduledoc """
  Edit v2 补丁层（docs/edit-v2-design.md §4.2/§5）：快照标签 + 原始行范围 + 最终内容。
  ...
  @doc "应用 v2 补丁，成功返回 %{status: :applied, files: [...]}。"
  def apply_patch(patch_text) do
    ...
    %{
      ...
      files:
        Enum.map(plans, fn p ->
          %{path: p.path, old_tag: p.tag, new_tag: p.new_tag, changed_lines: p.changed_lines, diff: p.diff, context: p.context}
        end),
      warnings: []
    }
  end
  ...
  defp plan_section(%{path: path, tag: tag, ops: ops}) do
    snap = SnapshotStore.fetch(path, tag) || raise RejectError, message: "未知快照标签 #{tag}（请先 show）", category: :unknown_snapshot
  ...
  defp write_section(plan), do: File.write!(plan.path, plan.new_content)
