defmodule Newbee.Colony.Workflow do
  ...
  defp finish_triage(data, root, summary) do
    case route_decision(summary) do
      {:ok, route, reason, participants} ->
        reason = if reason == "", do: "负责人判断范围明确，可直接实施。", else: reason
  ...
  # 模型的回复常有三种无害的偏差：整段被 ```json 包住、JSON 前后带解释、route 写成 SELF。
  # 这些仍是明确意图，不该让任务停在「判断投入」等人来点重试；真正含糊的文本仍然阻断。
  defp route_decision(summary) do
    text = summary |> to_string() |> String.trim()

    [text, unfence(text), json_slice(text)]
    |> Enum.uniq()
    |> Enum.find_value(:error, fn candidate ->
      case Jason.decode(candidate) do
        {:ok, %{} = map} ->
          with route when route in ["self", "discuss"] <- normalize_route(map["route"]) do
            participants = if is_list(map["participants"]), do: map["participants"], else: []
            {:ok, route, map |> Map.get("reason", "") |> to_string() |> String.trim(), participants}
          else
            _ -> nil
          end
  ...
  defp unfence(text), do: text |> String.replace(~r/\A```(?:json)?\s*|\s*```\z/u, "") |> String.trim()

  # 取「第一个 { 到最后一个 }」之间的片段：模型常把 JSON 包在解释里，嵌套对象也能覆盖。
  defp json_slice(text) do
    with {a, 1} <- :binary.match(text, "{") do
      rest = binary_part(text, a, byte_size(text) - a)

      case rest |> :binary.matches("}") |> List.last() do
        {offset, 1} -> binary_part(rest, 0, offset + 1)
        _ -> ""
      end
    else
      _ -> ""
