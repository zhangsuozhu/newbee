defmodule Newbee.Colony.Runtime do
  ...
  defp ensure_local_session(delivery, task) do
    with {:ok, colony} <- Store.get_colony(task["colony_id"]),
      ...
      error -> block(delivery, "执行会话启动失败：" <> format_failure(error))
    end
  end
  ...
  defp send_local(pid, delivery, task) do
    with {:ok, prepared} <- prepare(delivery, task),
      ...
      {:error, reason} -> block(delivery, "附件或投递失败：" <> format_failure(reason))
      _ -> :ok
    end
  end
  ...
  @doc """
  把投递失败原因转成用户能看懂的文案。
  LLM 的 HTTP/传输错误会走 Client.format_error（含状态码、provider 说明与后续动作），
  其余原因保持 inspect 以便排查；block/2 会把它写进任务 next_step，直接显示在工作卡上。
  """
  def format_failure(reason) when is_binary(reason), do: reason

  def format_failure({:http_error, _, _} = error), do: Newbee.LLM.Client.format_error(error)
  def format_failure({:upstream_error, _} = error), do: Newbee.LLM.Client.format_error(error)
  def format_failure({:stream_error, _} = error), do: Newbee.LLM.Client.format_error(error)
  def format_failure({:stream_error, _, _} = error), do: Newbee.LLM.Client.format_error(error)

  def format_failure(%Req.TransportError{} = error),
    do: Newbee.LLM.Client.format_error(error)

  def format_failure(other), do: inspect(other)
