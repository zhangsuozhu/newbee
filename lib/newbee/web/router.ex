defmodule Newbee.Web.Router do
  ...
  defp member_resource_gate(conn, _) do
    ...
    case resource do
      {sid, mode} ->
        if Newbee.Colony.Membership.session_access(token, sid, mode) == :ok, do: conn, else: unauthorized(conn)
  ...
  get "/ws" do
    ...
    case Newbee.Colony.Membership.session_access(token, sid) do
      :ok ->
        conn
        |> WebSockAdapter.upgrade(Newbee.Web.Socket, %{assigns: %{session: sid, token: token}}, timeout: :infinity)
        |> halt()
      ...
      case bearer_token(conn) do
        {:ok, token} ->
          case Newbee.Web.Auth.check_token(token) do
            ...
            {:error, _} ->
              if (String.starts_with?(conn.request_path, "/api/colony.") or
                    String.starts_with?(conn.request_path, "/api/session.") or
                    String.starts_with?(conn.request_path, "/api/upload/")) and
                   match?({:ok, _, _}, Newbee.Colony.Membership.authenticate(token)), do: conn, else: unauthorized(conn)
          end
