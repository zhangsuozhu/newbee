        ...
        "colony.view" ->
          ...
          with {:ok, view} <- Engine.view(cid) do
            ...
            if since != nil and since == rev do
              # 指纹一致：记录没变，回一个几百字节的轻量响应，前端跳过重渲染。
              {:ok, %{"changed" => false, "view_revision" => rev}}
            else
              members =
              ...
              tasks =
                Enum.map(view["tasks"], fn task ->
                  Map.put(task, "control_state", Newbee.Colony.Control.state(cid, "work", task["id"]))
                end)
