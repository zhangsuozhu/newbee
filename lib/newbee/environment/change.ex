defmodule Newbee.Environment.Change do
  ...
  **带 before/after 快照**。禁止把无关变更揉成"自动更新"。
  ...
  defstruct change_id: nil,
            ...
            approval_group: nil,
            approval_mode: nil,
            activated_revision: nil,
            retracted_at: nil,
            retraction_reason: nil,
            status: :requested,
            created_at: nil,
            updated_at: nil
  ...
  def new(attrs) do
    ...
    %__MODULE__{
      ...
      approval_group: Map.get(attrs, :approval_group),
      approval_mode: Map.get(attrs, :approval_mode),
      activated_revision: Map.get(attrs, :activated_revision),
      retracted_at: Map.get(attrs, :retracted_at),
      retraction_reason: Map.get(attrs, :retraction_reason),
  ...
  def to_map(%__MODULE__{} = c) do
    %{
      ...
      "approval_group" => c.approval_group,
      "approval_mode" => c.approval_mode,
      "activated_revision" => c.activated_revision,
      "retracted_at" => c.retracted_at,
  ...
  def from_map(m) when is_map(m) do
    %__MODULE__{
      ...
      approval_group: m["approval_group"],
      approval_mode: m["approval_mode"],
      activated_revision: m["activated_revision"],
      retracted_at: m["retracted_at"],
      retraction_reason: m["retraction_reason"],
