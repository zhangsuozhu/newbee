...
| 严格前置条件（stale） | 5 | Edit.V2 连续编辑同一文件 | sed 行号失效需重读 |
...
| Edit.V2 连续编辑忘 re-show（stale） | 高频 | sed -i 行号失效直觉 | 连续两次 patch 用旧 tag |
...
### 意见 H：编辑工具彻底停用 V1 锚点，统一 V2 行号模型
**违背的直觉**：sed -i '2c\new_line' file（按行号替换）、git apply（补丁带行号上下文）。模型对改文件的直觉是"指定行号，给新内容"。
**当前问题**：Edit.V1 用哈希锚点（N.#hash）——这是工具自创的抽象，语料里几乎没有对应物，模型必须从零学习（且当前还坏）。Edit.V2 用纯行号（PUT N..M:）——符合 sed 直觉。
**解决原理**：彻底停用 V1，只保留 V2。V2 的 stale 错误信息里加"文件已变化，请重新 show 获取最新行号"。
**为什么能改进**：V1 的锚点抽象在训练语料里无对应物，模型必须额外学习且当前无法正常工作。V2 的行号模型直接复用 sed 的肌肉记忆。
...
**解决原理**：在系统提示里附一个"各工具返回值形态速查表"（如 Edit.show → 裸 map、Run.sh → %{exit, output}、Edit.V2 → %{status:}），并强调"调用前先用 Introspect.exports 核实"。
