defmodule Newbee.Colony.WorkflowTest do
  ...
  test "harmless routing deviations (fence, prose wrapper, uppercase) still route", ctx do
    # 实测：任务正文写着「请只回复「收到」」时模型真的只回了「收到」，判定卡住。
    # 这里覆盖三种无害偏差；真正含糊的文本仍然阻断（见上一个用例）。
    root = create(ctx)
    fenced = "```json\n" <> ~s({"route":"self","reason":"仓库内局部改动","participants":[]}) <> "\n```"
    assert {:ok, executing} = complete(root, fenced)
    assert executing["workflow"]["phase"] == "executing"

    root2 = create(ctx, %{"title" => "大写 route"})
    wrapped = "我判断如下：" <> ~s({"route":"SELF","reason":"风格统一的小改动"}) <> "，随后实施。"
    assert {:ok, executing2} = complete(root2, wrapped)
    assert executing2["workflow"]["phase"] == "executing"

    root3 = create(ctx, %{"title" => "缺 reason"})
    assert {:ok, executing3} = complete(root3, ~s({"route":"self","participants":[]}))
    assert executing3["workflow"]["reason"] != ""
  end

  test "triage prompt ends with the output contract so task text cannot override it", ctx do
    root = create(ctx)
    text = Work.context(root, "请只回复「收到」")

    # 任务正文排在前面，契约必须钉在最后一行。
    assert String.ends_with?(text, "不要再输出其它文字。")
    assert String.contains?(text, "【本阶段输出契约 · 优先级最高】")
    assert String.contains?(text, ~s("route":"self"或"discuss"))
  end
end
